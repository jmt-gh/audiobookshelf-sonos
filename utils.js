...
async function buildProgress(libraryItem, updateObject) {
  ...
  let res = {
    ...
    bookDuration: audioFiles.map(audioFile => audioFile.duration).reduce((result, item) => result + item)
  }

  for (const audioFile of audioFiles) {
    let filename = audioFile.metadata.filename

    if (filename == partId) { // only grab as much duration as up to the part we are currently at
      break
    }
...
module.exports = { getMetadataResult, getMediaURI, updateAudioBookshelfProgress }
